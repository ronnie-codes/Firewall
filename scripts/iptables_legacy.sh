...
# Allow ping traffic
iptables -A FORWARD -i eth0 -o eth0 -p icmp -m state --state ESTABLISHED, NEW -j ACCEPT
iptables -A INPUT -i eth0 -o eth0 -d $DNS_IP -m state --state ESTABLISHED -p icmp -j ACCEPT
iptables -A OUTPUT -i eth0 -o eth0 -p icmp -s $MY_IP -m state --state ESTABLISHED,NEW -j ACCEPT
